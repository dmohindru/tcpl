...
int getch(void);
void ungetch(int);
void stackop(char command); /* Function to perform stack operations */
        ...
        switch (type) {
            ...
            case '%':
                ...
                if (op2 != 0.0)
                    ...
                    printf("error: zero divisor\n");
                break;
            case 'p':   /* print top two elements of stack */
                stackop('p');
                break;
            case '\n':
                printf("\t%.8g\n", pop());
...
/*stackop: perform stack operation like print, duplicate, swap and clear */
void stackop(char command)
{
    switch (command) {
        case 'p':   /* print top two elements of stack */
            if (sp - 1 >= 0)
            ...
            if (sp - 2 >= 0)
                printf("Second to top element: %g\n", val[sp-2]);
            break;
    }
}
